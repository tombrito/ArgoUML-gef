	...
	public void drawConnectingLine(Fig annotation) throws NullPointerException {
		...
		try {
			if (owner instanceof FigEdgePoly) {
				...
				line.setShape(annotation.getCenter(), AnnotationHelper.getClosestPointOnEdge(annotation.getCenter(),
						((FigEdge) owner).getFirstPoint(), ((FigEdge) owner).getLastPoint()));
			}
		} catch (ArrayIndexOutOfBoundsException e) {
			line.setShape(annotation.getCenter(), AnnotationHelper.getClosestPointOnEdge(annotation.getCenter(),
					((FigEdge) owner).getSourcePortFig().getCenter(), ((FigEdge) owner).getDestPortFig().getCenter()));
		}
		...
		AnnotationLineRemover.instance().removeLineIn(getAnnotationProperties(annotation).getLineVisibilityDuration(),
				annotation);
	}
