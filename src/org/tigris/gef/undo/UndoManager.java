...
public class UndoManager {
	...
	public void redo() {
		...
		do {
			...
			redo(memento);
		} while (redoStack.size() > 0 && !((Memento) redoStack.get(redoStack.size() - 1)).startChain);
		incrementUndoChainCount();
	...
	private void fireCanUndo() {
		...
		while (i.hasNext()) {
			...
			listener.propertyChange(new PropertyChangeEvent(this, "canUndo", "", getBoolString(undoChainCount > 0)));
		}
	}
	...
	private void fireCanRedo() {
		...
		while (i.hasNext()) {
			PropertyChangeListener listener = (PropertyChangeListener) i.next();
			listener.propertyChange(new PropertyChangeEvent(this, "canRedo", "", getBoolString(redoChainCount > 0)));
		}
	}
